                            ...
                            <div class="col-xs-12 col-sm-10 col-md-10 dashboard-content user-profile">
                                <div class="panel panel-default" style="margin-top: 25px;">
                                    <div class="panel-heading">
                                        <h4 style="display: inline; margin-right: 20px">{{trans('allclient.hq')}}</h4>
                                    </div>
                                    <div class="panel-body">
                                     <table class="table">
                                        <thead>
                                            <tr>
                                                <th style="width: 5%;">{{trans('allclient.STT')}}</th>
                                                <th>{{trans('allclient.hw')}}</th>
                                                <th>{{trans('allclient.he')}}</th>
                                                <th>{{trans('allclient.hr')}}</th>
                                                <th>{{trans('allclient.total')}}</th>
                                                <th>{{trans('allclient.ht')}}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{ ++$key }}</td>
                                                <td ><img src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->size(50)->errorCorrection('H')->generate($booking->booking_code)) !!} ">
                                                </td>
                                                <td>{{ date('H:i || d-m-Y ',strtotime($booking->created_at)) }}
                                                </td>
                                                <td>
                                                    @if($booking->status==0)
                                                ...
                                                <td><a href="{{ route('booking_detail',$booking->id) }}">{{trans('allclient.hp')}}</a></td>
                                            </tr>
                                            @endforeach
                                        </tbody>
                                    </table>
